...
import pylab as plt
import corner
from sersic_fitter import sersic_n1

def model(theta, radius):
    amplitude, effective_radius, offset = theta 
    model = amplitude * np.exp(-(2*1 -1./3) * ((radius/effective_radius)**(1.0/1) - 1.0)) + offset
    return model 
...
def lnprior(theta):
    amplitude, effective_radius, offset = theta 
    if (0 < amplitude < 100) and ( 0 < effective_radius < 100) and (0 < offset < 100):
        return 0.0
    return -np.inf
...
def fit_sersic1_mcmc(x, y, y_err):
    nwalkers = 1000
    niter = 5000
    data = (x, y, y_err)
    initial = np.array([np.max(y)/4, np.max(x)/4, np.min(y)])
    ndim = len(initial)
...
def sersic1_fit(x_data, y_data, y_err):
    x_plotting = np.linspace(x_data[0], x_data[-1], 1000)
    sampler, pos, prob, state = fit_sersic1_mcmc(x_data, y_data, y_err)
    ...
    plt.plot(x_plotting, sersic_n1(x_plotting, *best_params),color='r', lw=3, alpha=0.5, label = 'MCMC Best Fit')
    plt.legend()
    plt.show()

    labels = ['amp', 'reff', 'offset']
