"""Class used to calculate the surface density profiles """
...
from EstimateRMS import estimate_rms
...
def remove_stokes_data(fits_object):
...
class FitsImage():
	def __init__(self, fits_file):
		self.fits_file = fits.open(fits_file)[0]
		self.data = remove_stokes_data(self.fits_file)
		self.header = self.fits_file.header
		self.arcsec_per_pixel = self.header['CDELT2'] * ARCSECONDS_IN_DEGREE
		self.wcs = WCS(self.header, naxis = 2)

	def sum_annulus(self, center, data, radius_min, radius_max):
		xs = []
		ys = []
	...
	def sum_regions(self, center, radiis, sigma):
		areas = []
		sums = []
		uncertainties = []
		new_radiis = []
		for i in range(len(radiis)-1):
			current_sum, current_area, current_uncertainty = self.sum_region(
				center_pix, 
				radiis[i], 
				radiis[i+1], 
				sigma,
...
class Moment0(FitsImage):
	...
	def sum_region(self, center, radius_min, radius_max, sigma):
		sum_val, number_of_pixels = self.sum_annulus(center, self.data, radius_min, radius_max)
...
class HubbleImage(FitsImage):
	def __init__(self, hubble_file_name):
		...
		self.exptime = 4056.  # header is still the radio header. 
		self.data = self.data / 2.5  # dividing by the gain
...
def plot_CII(
	...
	axis.errorbar(
		arcseconds * arcsecondFactor, 
		y_vals / norm_factor, 
		yerr = y_uncertainties / norm_factor, 
		fmt = 'ro', 
		label = '[CII] Data', 
		capsize = 3, 
		ms = 2,
	...
	axis.plot(
		x * arcsecondFactor,
		sersic(x, *sersic_params) / norm_factor, 
		label = '[CII] Sersic Fit', 
		color = 'k'
		)
	...
	axis.fill_between(
		x * arcsecondFactor, 
		sersic(x, *sersic_params_lower) / norm_factor,
		sersic(x, *sersic_params_upper) / norm_factor, 
		alpha = 0.25, 
		)
...
if __name__ == '__main__':
	RC = Moment0('data/HZ7_Collapsed.fits')
	sigma = 2.3e-2
	#sigma = estimate_rms(RC.data)
	binwidth_arcsec = 0.2
	max_arcsec = 1.8
	center_pix = (155,139)
	...
	filters =[
	'hst_13641_07_wfc3_ir_f105w_sci_gaia_corrected_convolved_deresolved.fits',
	...
	center_pix = (152,141)
	hubble_sigma = 0.002128163
	effective_radiis = []
	for i, _filter in enumerate(filters):
		HI = HubbleImage(_filter)
		#hubble_sigma = estimate_rms(HI.data)
		radiis = calculate_pixel_radii(0.12, 1.5, HI.arcsec_per_pixel)

		HI.sum_regions(center_pix, radiis, sigma)
		vals, uncertainties, radiis = HI.sum_regions(center_pix, radiis, hubble_sigma)
		print('#############')
